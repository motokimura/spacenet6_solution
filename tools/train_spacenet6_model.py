...
from spacenet6_model.solvers import get_loss, get_lr_scheduler, get_optimizer
from spacenet6_model.utils import (
    checkpoint_epoch_filename,
...
def main():
    ...
    model, optimizer, lr_scheduler, start_epoch, best_score = load_latest_checkpoint(
        checkpoint_dir, model, optimizer, lr_scheduler, start_epoch, best_score
    )
    ...
    val_epoch = smp.utils.train.ValidEpoch(
        model, loss=loss, metrics=metrics, device=config.MODEL.DEVICE, verbose=True,
    )
    ...
    for epoch in range(start_epoch, config.SOLVER.EPOCHS):
        ...
        if epoch >= config.EVAL.EPOCH_TO_START_VAL:
            ...
            if best_score < val_logs[metric_name]:
                ...
                torch.save(
                    model.state_dict(), os.path.join(weight_dir, weight_best_filename())
                )
